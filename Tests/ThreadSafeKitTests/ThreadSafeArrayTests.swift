...
@Test(arguments: mechanisms) func threadSafeArrayAppendAndRead(mechanism: ThreadSafeMechanism) throws {
    let array = ThreadSafeArray<Int>(mechanism: mechanism)
    array.append(1)
    array.append(2)
...
@Test(arguments: mechanisms) func threadSafeArrayInitWithSequence(mechanism: ThreadSafeMechanism) throws {
    let array = ThreadSafeArray([1, 2, 3], mechanism: mechanism)
...
@Test(arguments: mechanisms) func threadSafeArrayIsEmptyFirstLast(mechanism: ThreadSafeMechanism) throws {
    let empty = ThreadSafeArray<Int>(mechanism: mechanism)
...
@Test(arguments: mechanisms) func threadSafeArrayPush(mechanism: ThreadSafeMechanism) throws {
    let array = ThreadSafeArray([2, 3], mechanism: mechanism)
    array.push(1)
    #expect(array.elements == [1, 2, 3])
}
...
@Test(arguments: mechanisms) func threadSafeArrayCodableRoundTrip(mechanism: ThreadSafeMechanism) throws {
    let array = ThreadSafeArray(["a", "b", "c"], mechanism: mechanism)
    let data = try JSONEncoder().encode(array)
    let decoded = try JSONDecoder().decode(ThreadSafeArray<String>.self, from: data)
    #expect(decoded.elements == ["a", "b", "c"])
}

// Auto-synthesized Codable on a containing type only compiles because
// ThreadSafeArray<String> conforms to Codable; this is the whole point of the feature.
@Test func threadSafeArrayCodableRoundTripInsideContainingType() throws {
    struct Container: Codable {
        let items: ThreadSafeArray<String>
    }
    let container = Container(items: ThreadSafeArray(["x", "y"]))
...
@Test func threadSafeArrayEquatableComparesElements() throws {
    #expect(ThreadSafeArray([1, 2, 3]) == ThreadSafeArray([1, 2, 3]))
    #expect(ThreadSafeArray([1, 2, 3]) != ThreadSafeArray([1, 2]))
    #expect(ThreadSafeArray([1, 2, 3]) != ThreadSafeArray([3, 2, 1]))
}

// Auto-synthesized Equatable on a containing type only compiles because
// ThreadSafeArray<Int> conforms to Equatable; this is the whole point of the feature.
@Test func threadSafeArrayEquatableInsideContainingType() throws {
    struct Container: Equatable {
        let items: ThreadSafeArray<Int>
    }
    #expect(Container(items: ThreadSafeArray([1, 2])) == Container(items: ThreadSafeArray([1, 2])))
    #expect(Container(items: ThreadSafeArray([1, 2])) != Container(items: ThreadSafeArray([1, 3])))
}

@Test func threadSafeArrayHashableUsableInSet() throws {
    let set: Set<ThreadSafeArray<Int>> = [ThreadSafeArray([1, 2]), ThreadSafeArray([1, 2]), ThreadSafeArray([3])]
    #expect(set.count == 2)
}
